# Density-independent Demography {#sec-did}
```{r include=FALSE}
source("scripts/setup.R")
```

In the preceding chapter, we listed Hastings' [@hastings2011ssp] key principles and assumptions of single species population growth. One of the key assumptions is that "all individuals in a population are identical." In this chapter, we elucidate an important violation of that assumption, population structure.

::: {#fig-human layout-ncol="3"}

```{r}
#| echo: false
include_graphics("figs/AgeDistM.png")
```

```{r}
#| echo: false
include_graphics("figs/AgeDistS.png")
```

```{r}
#| echo: false
include_graphics("figs/AgeSpecFert.png")
```

Demography of human populations of Mexico and Sweden. Based on 1990 data from US Census Bureau, Population Division, International Programs Center.
:::


*Populations have structure.* Consider the human populations of Mexico and Sweden in 1990. Mexico had a much larger fraction of their population in child bearing age classes or younger (@fig-human). In addition, the age-specific fertility rate was higher in Mexico, especially for younger women
(@fig-human). How did this happen, and why did Mexico have so
...
We can draw two different types of life cycle graphs for this two-stage model in just such a population (@fig-vdlc1). Some people find one more illuminating than the other. It is useful to be able to use both.
...
Two types of life cycle graphs. These both represent an amphibian pre-breeding model.
:::
...
(@fig-vdlc1) includes egg production, egg survival, tadpole or
...
\right)
$$ {#eq-m1}
If @eq-m1 represents the Western toad (@fig-vdlc1), then transition
...
In addition to fecundity, survival, maturation or growth from one stage to the next, some organisms undergo *regression* (@fig-conceptualmatrix). Regressing means to transition from a later stage to an earlier stage. For instance, and plant can shrink in size due to physical damage, disease or herbivory. A plant can also return temporarily to a non-reproductive stage after a large bout of reproduction. These are examples of regression (@fig-conceptualmatrix). 
...
A life cycle graph (Figs. @fig-toadlh, @fig-vdlc1) and the corresponding transition matrix @eq-m1 constitute our model. The matrix **A** for our structured population is directly analogous to $\lambda$ for our unstructured model of discrete population growth in the previous chapter.^[A is the analog of lambda.] 
...
Consider the matrix multiplication in @eq-exM.  We first multiply each element of row 1 of **A** ($a\; b$), times the corresponding elements of column 1 of **B** ($m\; n$), sum these products, and place the sum in the first row, first column of the resulting matrix. We then repeat this for each row of **A** and each column of **B**.
...
For our Western Toad (@fig-toadlh), this results in
$$
 \mathbf{A = GS + F} =
\left(
\begin{array}{cc}
...
Projection is very important for many reasons, especially for stochastic models or for very complicated models. However, we also get some of our best insights through direct analysis the transition matrix using *eigenanalysis* [@caswell2001mpm]. 
...
where $\mathbf{w}$ is a column vector with the same number of rows as $\mathbf{A}$. If we write out eq. @eq-eigsol for a $2 \times 2$ matrix, we would have
...
Let's use R to prove to ourselves that eq. @eq-eigsol is what we say it is. Here we see for ourselves whether $\mathbf{A}\mathbf{w} = \lambda \mathbf{w}$ for 
...
### Finite rate of increase {#sec-lambda1}
...
### Stable stage distribution {#sec-stable-stage-distribution}
...
```{r echo=TRUE, fig.cap="Calculating an annual finite rate of increase from a projected population shows that the population will approach asymptotically a constant geometric rate of increase. Solid points are annual growth rate; the circular point is the dominant eigenvalue, $\\lambda_1$."}
...
### Reproductive value {#sec-reproductive-value}
...
### Sensitivity and elasticity {#sec-sensitivity-elasticity}
...
Rely on expert advice [@caswell2001mpm; @ellner2006dmb; @kendall2019], and remember that the stages of life cycle graph and matrix are the stages that you collect at one point in time, and an arrow or transition element has to include everything that happens from one census period to the next.

## Integral projection
[Make sure `primer` and `tidyverse` packages are loaded.]
...
You will see below that when we implement integral projection models, we typically combine different techniques that are associated with both continuous variation (e.g., linear regression) and discrete variation (e.g., matrix algebra).
...
Probability density functions give us the unimodal curves that we are used to visualizing (e.g., the bell-shaped normal distribution). Cumulative probability functions are the integrals of these and allow us to calculate probabilities that a value will be in a particular region.
...
```{r cflh1, echo=FALSE, fig.cap="Smooth coneflower, *Echinecea laevigata*, is an endangered plant that lives in only a few places on poor soils. It enjoys open, sunny habitats, long walks in the rain, and reading a good book by the fire (prescribed or wild). Photo credit: Steve Croy", out.width='33%', fig.show='hold'}
include_graphics("figs/echinacea_laevigata_habitat_lg.jpg")
```
...
```{r fig-cflh2, echo=FALSE, fig.cap="Smooth coneflower, *Echinecea laevigata*, is an herbaceous perennial for which size seems to be a useful predictor of demographic rates.", out.width='100%', fig.show='hold'}
include_graphics("figs/coneflowerLH.png")
```

Our ultimate goal in this section is to do the following:

1. Given a plant of size *z* in year 1, predict whether it will survive to year 2.
1. If a plant survives to year 2, predict how big it is likely to be given that it was *z* in year 1, that is, $z^\prime$.
1. Given a $z^\prime$ plant, predict whether it flowers, and if it does, predict the number of seeds is it might produce.
1. Given a seed is produced, predict whether it will germinate, and guess its size.
...
Here we load data from the `primer` package, and give the data sets shorter names (because I am a slow typer).

```{r loadconeflower}
# library(primer) # if needed
...
Learn about these data using the help pages.
```{r}
# look up a help page on a data set
?coneflower
?coneflowerseeds
?coneflowerrecruits
```

Look at a bit of these data frames.
```{r}
glimpse(coneflower)
```
...
cfs <- dplyr::filter(cf, surv==1 & !is.na(logAp))
...
Typically, bigger individuals have a greater chance of flowering, and smooth coneflower is no exception. We estimate that chance of flowering using logistic regression.
...
One feature of these data is that most individuals do not flower at all and the even large individuals do not necessarily flower.
...
eda_pr <- ggplot(data = data.frame(x = c(0, 0.5)), aes(x)) +
  stat_function(fun = dbeta, n = 501, 
    ...
    labs(x="Prob(recruiting)", y="Probability density", title="Seed basket recruitment") +
  scale_y_continuous(breaks = NULL)
eda_pr
```

We can also describe the size distribution of new recruits, which are approximately log-normally distributed.
...
In the above section, we graphed data and used functions in `ggplot2` to fit some smooth curves. In this section, we actually use the statistical functions in R to make statistical models and hang on the model coefficients we will use to make quantitative predictions. These coefficients are the intercepts and slopes and uncertainties of the curves you saw above. I use code handed down on stone tablets from Ellner, Merow, and others.

Before we estimate everything, we create a single data frame to hold hold all of these estimates.
```{r params}
...
For survival probability, $s$, we use logistic regression, also known as a *general linearized model*^[Generalized linear models are a broad class of models that allow the use of a wide variety of types of data and error distributions.] with a binomial error distribution and a logit link function,
$$\log \left( \frac{s}{1-s} \right) = b_0 + b_1 z + \mathrm{error}$$
where $z$ is log-size, and  $0 \le s \le 1$ because it is a probability. The data for a logistic regression are ones and zeroes, or successes and failures, and $s$ is the estimate of the mean of the ones and zeroes at any given size.
```{r}
### survival regression
## the model statement
## family = tells us what kind of GLM to use.
surv.reg <- glm(surv ~ logA, data=cf, family=binomial())

## show us the coefficients b0 and b1
coef(surv.reg)
```

These are $b_0$ and $b_1$ from the above equation and allow us to describe the line and uncertainty^[In logistic regression, the uncertainty is a function of the mean probability of survival at each size.] we saw above in @fig-flowersurv. Next, we extract them and save them. 
...
where $z$ and  $z^\prime$ are log-leaf area in one year and the next year, and we estimate the error with the standard deviation around the fitted line.
```{r}
## Model statement
...
To estimate the probability that a plant flowers in a given year, we use survivors only and the presence or absence of flowers to fit a logistic regression.
```{r}
# Flowering
## model statement
...
## type params any time to look at the new values.
```
...
The point of making the projection kernel to automate the following steps:
...
To automate this, we make functions that will use the above statistical relationships and our model coefficients stored in the data frame `params` to make the kernel. Then we use those to fill out the kernel. The kernel is, in principle, continuous, but in practice is a matrix with very narrow size stages.
...
$$z^\prime \sim \mathrm{normal}\left(b_0 + b_1 z,\,\sigma^2\right)$$
where $z$ is the log-size in year 1, $b_0$ and $b_1$ are the intercept and slope of the linear regression twixt $z$ and $z^\prime$. The variance $\sigma^2$ is the uncertainty associated with the relationship between size in year 1 and sizxe in year 2, and we get it from the regression.
```{r}
...
Next we create a function to estimate the probability of having a particular "fecundity", which in these models is far more than just offspring production because it encompasses an entire year. It includes:

1. size-dependent flowering probability,
1. seeds per flowering head (maybe size dependent),
1. germination probability,
1. survival of new recruits from germination to first census, and
1. size distribution of new recruits.

Note that these comprise the sequence of life history events from whether a plant flowers or not, to the newest generation of seedlings and their variety of sizes. 
...
* the upper and lower size bounds (max, min) on log-plant size, and
* the number of size intervals, which determines
* the upper and lower size bounds (max, min) of each size interval,  
...
According to this code, our kernel will be a 100 x 100 matrix (`n = 100`).^[By definition a *density kernel* is continuous. The matrix we make uses many discrete categories to approximate a kernel, so we refer to it as a "discretized kernel".] Thus we are dividing up a log-size range of 2.32 (0.629--2.949) into 100 equal size categories or size stages. That is more finely divided than Collins and her colleagues actually used in their sizes estimates --- why would we do that? First, it ensures that we are not simplifying their data. Second, and more importantly, it allows us to make more accurate predictions for future populations: a mean is a more accurate prediction of a future observation, and is always at a higher resolution than the observations themselves.
...
## Survival kernel
S=s.z(y,params=params)   # 1 x 100 survival estimates
...
Finally, we plot the component and full kernels so that we can visualize them and thereby better appreciate what our results.
...
abline(h=c(1,1.5,2,2.5),lwd=.75)
abline(v=c(1,1.5,2,2.5),lwd=.75)
}
```
...
@caswell2001mpm is a definitive reference text for matrix models. @ellner2006dmb provides an excellent introduction, and I am sure there are many other excellent texts as well.

@stubben2007ead and @stott2018pdm, and @delacruz:2019 provide R packages for stage-based matrix models with methods that go well beyond this text. In addition, there are several other packages for analyzing life tables and human demography.
...
1. Draw by hand two different types of life cycle graphs (e.g., @fig-garlic1 and @fig-cflh2) for this loggerhead population. Include the matrix elements associated with each transition. 
1. Use eigenanalysis to determine $\lambda_1$.  Explain what this tells us about the population, including any assumptions regarding the stable stage distribution.
1. Use eigenanalysis to determine the stable stage distribution.
1. Use eigenanalysis to determine the elasticities. Which transition(s) are most influential in determining growth rate?
1. What is the predicted long-term relative abundance of all stages? What do we call this?
1. If your interest is to maximize long-term growth rate, in which stage(s) should you invest protection measures? Which stages are least likely to enhance long-term growth rate, regardless of protective measures?
1. Start with $\mathbf{N} = \left(1000\,10\,10\,10\,10\,10\,10\right)$ and graph dynamics for all stages for 20 years.
