...
```{r}
#| label: fig-theorydatanathist
#| fig.cap: Ecology and evolutionary biology require all of these to advance. 
#| fig_width: 2
#| column: margin
#| 
g <- dagitty(
...
and mechanistic view of how the world works [@scheiner2010cfb]. Theories
...
(2017))](figs/Rose_et_al_2017_fig_1.png){width="80%"}

An early and persistent organizing schema in ecology is based on
*hierarchy theory* [@oneill1986; @rose2017hff, and references therein]. It
...
## An example: Metabolic Theory of Ecology {#sec-metabolic-theory-ecology}
...
[@brown2004mte] is a well-supported body of knowledge about the underlying
...
*quarter power scaling* [@brown2004mte]. When we compare the basal (i.e.
...
scale in the same manner [@niklas2001isr], although size or mass is a
...
```{r}
#| label: fig-MTE34
#| fig.cap: Metabolic rate increases predictably with species body sizes. 
#| column: margin
#| echo: false
...
```{r}
#| label: fig-MTE-MS
#| fig.cap: Mass-specific metabolic rate decreases predictably with species body sizes. 
#| column: margin
#| echo: false
#| 
...
group including Jim Brown and Geoffrey West [@west1997gmo] proposed an
...
[@banavar1999sfe; @banavar2002mus], arguing that the assumption of the
...
theory of ecology [@brown2004mte]. In 2000, they suggested that the
...
metabolic rate with increasing temperature (@fig-arrh).

```{r}
#| label: fig-arrh
#| fig-cap: The effect of body temperature on ectothermic metabolic rates can be approximated with the Arrhenius function, $B = a e^{-E_a/(kT)}$. Here $a = 10^4$, and $E_a = 0.23$. It is similar in shape to a power law with $z > 1$, over the range of biologically relevant temperatures.
#| column: margin
...
predictions regarding all kinds of ecology phenomena [@brown2004mte],
including
...
```{r}
#| label: fig-logscale
#| fig-cap: Changing the scales of the axes to linearize power law relations. Note scales are logarithmic, using the original linear values.
#| column: margin
# using base R
...
most abundant to the least abundant (@fig-mete1a).

```{r}
#| label: fig-mete1a
#| fig-cap: Rank abundance distribution of tree species in a 50 ha plot on Barro Colorado Island. Line is a best fit of the log-normal distribution.
#| column: margin
library(vegan)
data(BCI)
abun <- colSums(BCI)
...
The curve formed by these data points (@fig-mete1a) is
...
```{r}
#| label: fig-mete1b
#| fig-cap: Species abundance distribution of tree species in a 50 ha plot on Barro Colorado Island. Line is a best fit of the log-normal distribution.
#| column: margin
...
abundances, we get the \*species abundance distribution (@fig-mete1b). @preston1962cdc noted that the distributions were remarkabling similar to a normal
...
[@hubbell2001vvv]. Later, John Harte showed that the principle of maximum
...
```{r}
#| label: fig-mete2
#| fig-cap: Expected rank abundance distribution, given five individuals and three species, under the maximum entropy theory of ecology.
#| column: margin
