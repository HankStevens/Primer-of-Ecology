...
umut.flowfield <-  flowField(mutualism.LV, xlim = c(0, 15), ylim = c(0,15),
                             ...
                             ylab="N2", xlab="N1", main= bquote(italic(r) < 0))
...
umut.flowfield <-  flowField(mutualism.LV, xlim = c(0, 15), ylim = c(0,15),
                             ...
                             ylab="N2", xlab="N1", 
                             main=bquote(abs(alpha[ii])<alpha[ij]))
umut.nullclines <- nullclines(mutualism.LV, xlim = c(0, 15), ylim = c(0,15), 
...
In this section, we will model mutualisms using a framework that, unlike our Lotka-Volterra approach, is intended specifically to describe resource consumption by one or both partners [@macarthur1972Geographical]. 
...
```
This shows us we will not get coexistence in the absence of soil flora. 

Now we add the soil flora.
```{r}
p <- list(ra=0.7, rb=0.5, Ka = 10, Kb=12, cA=.98, cB=.98, 
