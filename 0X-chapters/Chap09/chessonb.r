...
chesson <- function(alpha=c(2*1e-5, 1e-5), d=.1, years=10, N0=c(1e3,1e5),
                    w=c(.5, 1), env.var=1,
...
w.rare <- w[1]
w.comm <- w[2]
b.r <- specialization + 1
mode.r <- .5 + spread/2
a.r <- (mode.r*(b.r - 2) + 1)/(1-mode.r)
...
for(i in 1:years) Ns[i+1,] <- { 
 ...
 return(list(time=c(0,t), Ns=Ns, Es=Es, Cs=Cs, Rs=Rs, Bs=Bs,
             env=env.beta-.5,
             params=c(a.r,b.r,a.c,b.c,spread, rho[["value"]])))
}
