...
In the following comparison between mutualist and antagonist bipartite networks, we control for network structure, and ask whether the subsequent effects of diversity, connectance, nestedness, and modularity on resilience differ between mutualist and antagonist networks (Fig. \@ref(fig:TandF)). In brief, you will find that they do.  
...
To use `bip_stability` and analyse the results, you will need to install a few more packages.  If you've already installed them, you have nothing to do.
```{r}
# pkgs <- c("bipartite", "igraph", "rARPACK", "lavvan", "semPlot")
# install.packages(pkgs)
```

This package has several arguments which you can see described in the help file. Among them are
...
```{r}
library(primer)
b <- bip_stability(Spp.a = 16:60, Spp.p = 8:30, C=c(.05, 0.5), reps=100, quant=FALSE)
```
...
To make these figures, you'll need the `semPlot` package.
```{r paths, fig.cap="*Given the same bipartite network structure and the identical interaction strengths, mutualistic (left) and antagonistic webs (right) show different relations among emergent properties.*", fig.show='hold', out.width="50%"}
# library(semPlot)
...
The differences lie in the effects on resilience. What are those differences, and are they consistent with Thebault and Fontaine (2010). Are they consistent with what is generally presumed about the role of mutualisms and consumption in food webs?
