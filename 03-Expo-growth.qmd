# Simple density-independent growth {#sec-expo}
```{r include=FALSE}
source("scripts/setup.R")
```

```{r fig-Melospiza1, echo=FALSE,  fig.cap="Song Sparrow (*Melospiza melodia*) counts in Darrtown, OH, USA. From  Sauer, J. R., J.E. Hines, and  J. Fallon. 2005. The North American Breeding Bird Survey, Results and Analysis 1966--2004. Version 2005.2. USGS Patuxent Wildlife Research Center, Laurel, MD.", out.width="70%"}
data(sparrows)
ggplot(data=sparrows[1:6,], aes(x=Year, y=Count)) + geom_line() + geom_point()
``` 

```{r fig-Melospiza2, echo=FALSE,  fig.cap="Song Sparrow (*Melospiza melodia*) annual changes in population size as a function of population size.", out.width="70%"}
spar1 <- tibble(Count = sparrows$Count[1:5], ssgr= with(sparrows[1:6,], Count[2:6]/Count[1:5]))
ssgr <- with(sparrows[1:6,], Count[2:6]/Count[1:5])
ggplot( data=spar1, aes(Count, ssgr) ) + geom_point() +
         labs(y=bquote(N[t+1]/N[t]) )
``` 



Between 1966 and 1971,\index{Song Sparrow} Song Sparrow (*Melospiza melodia*) abundance in Darrtown, OH, USA, seemed to increase very quickly, perhaps unimpeded by any particular factor (Fig. @fig-Melospiza1,  @fig-Melospiza2). In an effort to manage this population, we may want to predict its future population size. We may also want to describe its growth rate and population size in terms of mechanisms that could influence its growth rate. We may want to compare its growth and relevant mechanisms to those of other Song Sparrow populations or to other passerine populations. To do this, we start with the simplest of all population phenomena, geometric and exponential growth.

[In this chapter]{.small-caps}, we'll introduce density-independent population *projection*, *growth*, and *per capita growth*, for populations with synchronous reproduction and non-overlapping generations (discrete models of geometric growth), and continuous reproduction with overlapping generations (continuous models of exponential growth). 

## Learning goals

- recite and explain the provided theoretical principles of population growth.
- derive geometric and exponential per capita population growth, population growth, and the projection equations.
- define and illustrate density-independence.
- describe an algorithm to generate a stochasticly fluctuating population.
- explain $1/f$ noise (colored environmental variation), its relevance for natural populations, and its implications for population dynamics.

## Principles and assumptions

Geometric and exponential growth are examples of *density-independent growth*. We define *density-independent population growth* as growth that results in a population that lacks a statistical relation between the density of a population, and its *per capita* growth rate^[No observed relation between $\frac{\Delta N_i}{N_i}$ vs. $N_i$]. This is an operational and practical definition that we can apply easily to empirical data. We demonstrate actually testing for it in @sec-dd-test.

Geometric^[A geometric series is one with a constant ratio between successive numbers.] and exponential^[An exponential series is one described by $\exp^{a}$ where $a$ is constant.] growth capture the fundamental process of biological reproduction. For instance, one cell divides to make two, those two cells each divide to make four, and so on, where reproduction for each cell results in two cells, *regardless of how many other cells are in the population*---that is what we mean by *density-independent*. This myopically observed event of reproduction, whether one cell into two, or one plant producing many seeds, is the genesis of a geometric series. With overlapping generations, this series becomes exponential. Therefore, most models of populations include this fundamental process of increase. 

While density-independent growth can't be maintained for long periods of time, populations can grow this way while resources are plentiful and nothing else limits their growth. It behooves us to start with this simple model because more complex population models almost always include this process.

@hastings2011ssp proposes that we approach single species poulation growth from either a microscopic or macroscopic point of view. The microscopic approach begins with two propositions. The first is that if we know the location, timing, and traits of all individuals, we can predict perfectly population dynamics; the second is that we can never predict dynamics perfectly because births and deaths are fundamentally random and can be described only with probabilities.^[A zen koan, if there ever was one.] With this microscopic approach, we would seek a very detailed description of individuals and build a complex model to understand the consequences of the characteristics of all these interacting individuals, including the growth of the population.
...
The first of these is the subject of *this* chapter.

These are consequences of the following assumptions.

a. all individuals in a population are identical.
b. there is no migration in or out of the population.
c. the number of offspring per individual (or the per capita birth and death rates) are constant through time, and independent of the number of individuals in the population.^[We might also assume that a. implies c.]

These conditions are sufficient^[$X$ is sufficient to cause $Y$ if $Y$ always follows $X$, even if $Y$ also has other causes.] to cause exponential growth, and they are widely described as *the* assumptions underlying exponential growth. However, they are not actually necessary^[$X$ is necessary for $Y$ if $Y$ never occurs without $X$]. For example, the following conditions are sufficient to generate exponential growth.

- individuals are identical and immigration is equal to emigration, or
- individuals differ, but if the relative abundance of different types remains constant (@sec-did), and immigration is equal to emigration.

Regardless of any underlying conditions, we say that a population is growing exponentially if it follows an exponential trajectory. It can be as simple as that.

Nonetheless, deviations from underlying assumptions lead to all of the most interesting parts of single species population dynamics [@hastings2011ssp]. 

At the end of the chapter, we relax assumption c. We will model stochastic variation in population growth rate to investigate extinction risk. 



## Discrete growth
This section covers models models of discrete non-overlapping generations.

### A story of fruit flies in my kitchen
...
For now, we'll treat fruit flies as if they grow in discrete generations. This is very common for populations that live in seasonal habitats - their reproduction is timed to the season, and they breed altogether in one bout.^[discrete generations are common in nature.]

I count the number of flies every week, and I find these numbers [@tbl-flies].

```{r}
#| label: tbl-flies
#| echo: true
#| tbl-cap: "The number of fruitflies in my kitchen increased over four weeks."
t <- c(0, 1, 2, 3)
N <- c(2, 4, 8, 16)
d <- tibble(t, N)
d |>
...
#| label: fig-flies
#| echo: true
#| fig-cap: "The number of fruitflies in my kitchen increased geometrically."
#| column: margin

# base graphics assumes R sees t, N
# plot(x = t, y = N, type = "b" ) 

## ggplot graphics uses dataframes
ggplot(data=d, aes(x=t, y=N) ) +
  geom_line() + geom_point() + theme_bw()
```


There are several ways we can describe fruit fly population growth (@fig-flies-model).  Here, we choose to begin by considering the proximate causes of change to population size per unit time: the numbers of births, immigration, death and emigration. Those are the only options, and we state it thus:
$$\frac{\Delta N}{\Delta t} = \frac{B + I - D - E}{\Delta t}$$
that is, the change in numbers of individuals per unit time^["Delta" N is the change in N.] is determined by the numbers of births, deaths, and migrants per unit time. We refer to this as an *open* population because it includes migration into or out of the population.
...
If I counted the number of flies last week and again this week, I can calculate the change ($\Delta N$) over one week We have 
$$ \frac{N_{t+1} - N_t}{(t+1) - t}=\frac{\Delta N}{\Delta t}=\frac{B+D}{\Delta t}$$
In this equation, $t$ is an arbitrary week, so $t+1$ is one additional week. 

```{r}
#| label: fig-flies-model
#| echo: false
#| fig-cap: The number of fruit flies in my kitchen depends on immigration and emigration, and births and deaths. All rates are individuals per unit time. If $I,\,E = 0$, this would be a closed population.
#| out-width: "100%"
#| fig-width: 5
#| fig-height: 5
nodes <- c("I", "F", "D", "E")
g1 <- make_graph( edges=c(1,2, 2,3, 2,2, 2,4), n=4, directed=TRUE ) 
l <- cbind(c(-1, 0, 1, .83), c(0, 0, 0, -.4))
V(g1)$name <- c("pool 1", "Flies", "pool 2", "pool 3")
V(g1)$size <- 40
V(g1)$shape <- c("none", "circle", "none", "none")
E(g1)$label <- c("I", "E", "B", "D")
E(g1)$label.y <- c(.05, .05, -.4, -.1)

plot(g1, layout=l, loop.size = 1, edge.loop.angle=3*pi/2, 
     margin=c(0,0,0,0), rescale=FALSE) 
```
### From story to model

From numbers of flies, we derive per capita rates, and then we apply those rates sequentially for births and deaths.

Because adults give rise to offspring, we can represent births and deaths as proportions of existing adults. We calculate the average per capita birth and death rates as,
$$\frac{B}{N} = b;\quad \frac{D}{N}=d$$
This reflects the biological realities that adults produce offspring, or $bN = B$. Also, everyone has some chance of dying $dN = D$. The per capita birth rate $b$ can be any non-negative real number, $b \ge 0$. This model of births reflects the geometric property of reproduction: over a specified time interval $\Delta t$, an average parent makes $b$ babies. Per capita death rate $d$ is any real number between zero and one, $0 \le d \le 1$. Both $b$ and $d$ have units of individuals per individual per unit time. They depend on that unit of time.

From @fig-flies-model, we create an algorithm for creating a model of density dependent growth:

1. A proportion flies in the starting population reproduce, and thereby increase the size of the population, and then
1. A proportion of flies from both the starting population and newly hatched flies die, decreasing the size of the population.

Let,

* $N_0$: size of the original population; the number of flies in the first week.
* $N^\prime$: size of the population immediately after births, 
* $N_1$: size of the population after births *and* deaths; the number of flies in the second week.

We can symbolize our algorithm thus:

1. $N^\prime = N_0 + bN_0 \quad \text{number of adults plus births}$
1. $N_1 = N^\prime - dN^\prime \quad \text{(number of adults plus new flies) minus deaths}$


We see that by the next time point, $t=1$, the number of fruit flies is predicted to be equal to the number we started with, $N_0$, plus the number of new individuals, $bN_0$, minus the number of original adults that die, $dN_0$, and minus the number of new individuals that die, $dbN_0$.

To get from one year to the next in a single step, we combine the above two equations into one through substitution and simplify:

\begin{align*}
N_1 &= N^\prime - dN^\prime\\
 & = N_0 + bN_0 - d\left(N_0 + bN_0\right)\\
 & = N_0 + bN_0 - dN_0 - dbN_0 \\
 & = N_0 \left(1+b - d - db\right)\\
 & = N_0 \left(1+b - d - db\right)\\
 & = N_0 \left(1+r_d \right) = N_0 + r_d N_0\\
 & = N_0 \lambda
\end{align*}

where $\lambda$ is the standard symbol for this rate.  We refer to $\lambda$ as the *finite rate of increase* and $r_d$ as the *discrete growth increment*.

**Recapping**, we now have the machinery to describe the rate of change, and proven that it can be derived from per capita birth and death rates.

### Growth rate equations

The *population growth rate* is $\Delta N / \Delta t$, where, in our case $\Delta t = 1\,\text{wk}$. At $t=0$, we can derive it as,

$$\frac{\Delta N}{\Delta t} = \frac{N_1 - N_0}{t_1-t_0} = \frac{(N_0 + r_dN_0) - N_0}{t_1-t_0} = \frac{r_d N_0}{1}=r_d N_0$$

If we generalize, we drop the zero, to get $r_dN$. The *per capita* population growth rate is $r_dN/N =r_d$.

### Projection equation

In this simple model with a constant rate of increase, we can project the population *size* directly. Let's derive the equation via induction (i.e. generalizing from a pattern).

Consider that we know that 

$$N_1 = N_0\lambda$$
regardless what $N_0$ is. Therefore, we extend this logic to another year,

$$N_2 = N_1\lambda=  (N_0 \lambda)\lambda$$
and then another,
$$N_3 = N_2\lambda=  (N_0 \lambda)\lambda\lambda$$
in which we keep multiplying by $\lambda$ successive times.

You probably notice, then, that to get from $t=0$ to an arbitrary time $t$ in the future, we infer,

$$N_t = N_0\lambda^t$$
where $t$ is some number of years after $t=0$.

## Continuous growth 

In the reality that is my kitchen, individual fruit flies are having sex and reproducing on their own schedules. As a population, they breed continuously, so the cohorts are *not synchronous* and *generations overlap*. For populations like that, we should describe *instantaneous* growth rates, rather than $b$ and $d$ being annual increments as in the previous section. We estimate these as $t$ gets really small, that is, $t \rightarrow 0$---this allows us to estimate an *instantaneous* rate.
...
Now we integrate both sides of the separated equation. On the left, the variable of integration is $N$, running from $N_0$ to $N_t$. On the right, we integrate with respect to time -- but $t$ is already doing double duty as the *upper limit* of that interval, so using it as the integration variable too would mean the same symbol names both a fixed endpoint and a value that varies as it's summed over. To avoid that clash, we introduce a dummy variable $\tau$ ("tau") for time as it runs from $0$ up to $t$; $\tau$ is just a placeholder that disappears once the integral is evaluated. This is the method of *separation of variables*.
$$\int_{N_0}^{N_t} \frac{1}{N}\,dN = \int_{0}^{t} r\,d\tau$$
Evaluating each side's antiderivative at its bounds (the Fundamental Theorem of Calculus),
$$\Big[\ln|N|\Big]_{N_0}^{N_t} = \Big[r\tau\Big]_{0}^{t}$$
$$\ln|N_t| - \ln|N_0| = rt - r(0) = rt$$
Since $N$ is a population size, $N > 0$ always, so the absolute value bars are unnecessary and we can drop them:
$$\ln(N_t) - \ln(N_0) = rt$$
$$\ln(N_t) = \ln(N_0) + rt$$
...
## Properties of geometric vs. exponential growth {#sec-lambda-r-relation}

Compare the projection equations for geometric and exponential growth: discrete growth is described in terms of $\lambda$, continuous growth in terms of $r$.

| | Discrete (geometric) | Continuous (exponential) |
|:--|:--:|:--:|
| **Projection** | $N_t = N_0\lambda^t$ | $N_t = N_0 e^{rt}$ |
| **Population growth rate** | $\dfrac{\Delta N}{\Delta t} = r_dN$ | $\dfrac{dN}{dt} = rN$ |
| **Per capita population growth rate** | $\dfrac{\Delta N}{N\Delta t} = r_d$ | $\dfrac{dN}{N\,dt} = r$ |

: Discrete vs. continuous population growth {#tbl-lambda-r}

where $\lambda = 1 + r_d$. In both cases, the per capita growth rate does not include $N$ -- this is what makes both of these models of *density-independent* growth.
...
```{r echo=TRUE}
N <- c(20, 30, 15, 15)
# R where is the ratio of successive population sizes, i.e.,
# the realized $\lambda$ for just one year
R <- N[2:4]/N[1:3]; R
```
...
$$N_3 = (N_0 R_0) R_1 R_2$$
where $R_t$ is the realized $\lambda$ for just one year.
...
N[1]*R[1]*R[2]*R[3]
```
From this calculation, we see that when we start with $N_0=20$ and do the annual steps, we wind up with a smaller population, even though the arithmetic average is $R_{\mathrm{ave}} = 1$. How do we calculate an average of numbers that we multiply together, rather than add?

We want a number $\bar{R}$ such that 
$$\bar{R}^t = R_1R_2\ldots R_t$$

What value of $\bar{R}$ will give us that result?

To find that, we simply solve for $\bar{R}$, where
...
Now imagine that rate varies each year, around the average of 1.01, and a standard deviation of 0.3.
```{r}
#| label: fig-lvary
#| fig-cap: "Random lambda"
#| column: margin

set.seed(5)
n.lambdas <- 100
lambda_varying <- rnorm(n.lambdas, m=lambda, sd=0.3)
hist(lambda_varying)
```

Now let's compare the arithmetic and geometric means of these random values.
```{r}
mean(lambda_varying)
prod(lambda_varying)^(1/n.lambdas)
```

Another way to calculate the geometric mean is to take the arithmetic mean of the log-lambdas, and then exponentiate the result.
```{r}
exp( mean( log( lambda_varying)))
```

The geometric mean will always be less than the arithmetic mean.

## Simulating dynamics, with environmental noise
...
Given these sources of uncertainty, @shaffer1981mps defines *minimum viable population* size (MVP) thus,
...
Now we plot these counts through time (Fig. @fig-cts).  
```{r fig-cts, fig.cap="Observations of Song Sparrows in Darrtown, OH (http://www.mbr-pwrc.usgs.gov/bbs/).", out.width="75%"}
...
```{r}
#| label: sim-obsR
# create a new dataframe that contains everything in sparrows and the 
# new variable, obs.R
sparrows2 <- sparrows |>
  mutate( # change variables
    obs.R = lead(Count)/Count # groovy way to calculate N[t+1] / N[t]
    # Count / lag(Count) would give us N[t] / N[t-1]
    ) |>
  drop_na() # remove the row with obs.R equal NA (last row)
```
...
```{r fig-Rrates, fig.cap="Annual growth rates (R=N[t+1]/N[t]) for Song Sparrows", out.width="75%"}
ggplot(data=sparrows2, aes(x=Year, y=obs.R) ) + geom_point() + 
  geom_hline(yintercept=1, lty=3) + 
  labs(y=bquote(N[t+1]/N[t]), x="Year (t)")
```
One thing that emerges in our graphic data display (Fig. @fig-Rrates) is we have an unusually high growth rate in the early 1990's, with the rest of the data clustered around 0.5--1.5. We may want to remember that.
...
(rRs <- sample(x=sparrows2$obs.R, size=years, replace = TRUE))
```
...
```{r}
#| label: lastcount
N[1] <- sparrows$Count[length(sparrows$Count)]
```
...
```{r }
#| label: fig-simten
#| fig-cap: "A single simulated population projection."


plot(0:years, N, type="b") # base graphics
d <- tibble(Year=0:years, N)
d |>
  ggplot(aes(x=Year, y=N)) + 
  geom_line() + geom_point()
```

It appears to work (Fig. @fig-simten). 
...
outmat <- replicate(sims,   
             expr=myForLoop(obs.R=sparrows2$obs.R, years=years, initial.N=43)
                    )
```
Now let's peek at the results (Fig. @fig-tensimfig). It is fun to graph our output, but also helps us make sure we are not making a heinous mistake in our code. Note we use log scale to help us see the small populations.
```{r fig-tensimfig, fig.cap="Using matplot() to plot a matrix vs. a single variable. Our simulated populations sometimes increase and sometimes decrease.", out.width="75%"}
matplot(0:years, outmat, type="l", log="y")
```


```{r fig-tensimfig2, fig.cap="Using ggplot() to plot one variable against vs. a single variable, organized by a grouping variable. Our simulated populations sometimes increase and sometimes decrease.", out.width="75%"}
...
A Bayesian approach forces us to use what we already know. For us, we will start with expert opinion of whether Song Sparrows are likely to maintain a self-replacing population (a 'source' population - @sec-source-sink-model) and avoid extinction over our time interval. In other words, we ask Dr. Dave Russell. He says "Probably, but you can't be sure." Let's represent "probably" with the beta distribution. The **beta distribution** generates probability densities for a random variable between [0,1], perfect for representing probabilities which always lie between zero and one. The beta distribution has two parameters, often called shape 1 ($\alpha$) and shape 2 ($\beta$). The mean of the beta distribution is,
$$\mu = \frac{\alpha}{\alpha + \beta}$$

```{r fig-survivalPrior, fig.cap="Our prior belief on the probability that the Song Sparrow population in Darrtown will replace itself over our time interval. We call this the *prior probability distribution.*", out.width="67%"}
...
Fig. @fig-survivalPrior shows our *prior beliefs*. That is, before we look at our data, we think there is about a 3/5's chance (60%) that Song Sparrows in Darrtown won't become extinct over our time interval. How will we update our beliefs using our data? 
...
We can fit a normal distribution to the logarithms of our observed $R$, and we see that it doesn't do too bad a job (Fig. @fig-fitln).

```{r}
#| include: false
mu <- mean( log(sparrows2$obs.R) )
sigma <- sd( log(sparrows2$obs.R) )
...
::: {#fig-fitln layout-ncol="2"}

```{r}
hist(log(sparrows2$obs.R), breaks=10, ylab="Frequency")
lines(lR, rdR)
```

```{r}
qqnorm(log(sparrows2$obs.R)); qqline(log(sparrows2$obs.R))
```

The logarithms of the observed R arrayed in a histogram (left) and a quantile-quantile plot comparing our log-growth rates to the normal distribution (right). Our data seem somewhat approximated by a Normal distribution whose mean and standard deviation are derived from the log-transformed data. The probability distribution has been rescaled to be visible on this graph. However, the q-q plot shows that the sample (y-axis) includes values that are smaller and larger than predicted by the normal distribution.
:::
...
Environmental variables typically have a reddened color that we often call pink noise [@halley1996ee1].This type of environmental variation as *environmental color*, whether white, pink, brown, or even, in the other direction, blue. 
...
```{r fig-specex, fig.cap="Example of spectral mimicry, showing the original random data points (solid black line), the template ('one.over.f.template', red dashed line), and the rearranged, reddened data points (solid red line).", outwidth="67%"}
...
In Fig. @fig-specex, the solid lines refer to the uniform data (black) and the reddened (red).
...
R.n = length(sparrows2$obs.R) # number of observations
...
R.f <- spec_mimic(sparrows2$obs.R, one.over.f.template)

## combine and plot
series <- cbind(sparrows2$obs.R, one.over.f.template, R.f)
