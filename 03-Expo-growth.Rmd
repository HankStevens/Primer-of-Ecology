...
before maturing. Many of the adults die as well. If we assume the eggs, larvae, and adults all die at the same rate, then by the end of one generation we have $N_1 = dN^\prime = d(N_0 + bN)$. 
...
First, we convert persisting populations to 1, and extinct populations to zero. We then sum the ones and divide by the total number of all simulated populations. This gives us the fraction surviving.
```{r}
persisting <- as.numeric( N.2053 > 1 )
(p <- sum(persisting)/length(persisting))
```

Next we can ask what the uncertainty is associated with that? We will find an intuitive Bayesian answer to the question: What is the probability that the sparrow would become extinct in this region, due to these inherent processes?

We will gain an intuition into Bayesian probability elsewhere. For now, we will simply rely on well-established relationship between the binomial probability distribution and the beta distribution.

We start with our expert opinion of whether Song Sparrows are likely to maintain a self-replacing population (a 'source' population - Ch. 6) and avoid extinction over our time interval. In other words, we ask Dave Russell. He says "Probably, but you can't be sure." Let's represent that with the beta distribution. The **beta distribution** generates probability densities for a random variable between [0,1], perfect for representing probabilities. It has two parameters, shape 1 ($\alpha$) and shape 2 ($\beta$). The mean of the beta distribution is,
$$\mu = \frac{\alpha}{\alpha + \beta}$$

```{r survivalPrior, fig.cap="Our prior belief on the probability that the Song Sparrow population in Darrtown will replace itself over our time interval.", out.width="67%"}
...
Fig. \@ref(fig:survivalPrior) shows our *prior beliefs*. That is, before we look at our data, we thinking there is a 3/5's chance (60%) that Song Sparrows in Darrtown won't become extinct over our time interval. How will we update our beliefs using our data? 
...
```{r}
nR <- length(obs.R)
nR.surv <- nR * sum(persisting)/length(persisting)
nR.ext <- nR - nR.surv
```
...
When we model or simulate time series, we often find it essential to account for patterns that arise on different time scales and have different magnitudes. For instance, air temperature (and its consequences) varies diurnally, from day to day and week to week, and across seasons. We often refer to this as "one over f-noise" ($1/f$-noise), that is, random variation multiple overlapping asynchronous patterns of variation can be characterized with *spectral analysis* [@Shumway2019; @Halley1996]. Spectral analysis allows us to detect cyclic repeating patterns in noisy data, such as the El Niño Southern Oscillations, small mammal population cycles, tree masting dynamics, or disease outbreaks.
...
Environmental variables typically have a reddened color that we typically refer to as pink noise [@Halley1996]. We refer to this type of environmental variation as *environmental color*, whether white, pink, brown, or even, in the other direction, blue. 
...
* a template pink (slightly reddened) sequence,
* a rearrangement of the original string to match the spectral properties of the template.

We then plot the three vectors.
```{r specex, fig.cap="Example of spectral mimicry, showing the original random data points (solid black line), the template ('one.over.f.pattern', red dashed line), and the rearranged, reddened data points (solid red line).", outwidth="67%"}
...
N.random <- runif(N)
## Pink noise template
one.over.f.pattern <- one_over_f(gamma=1, N=N)
## Rearrangement (mimicry)
N.f <- spec_mimic(N.random, one.over.f.pattern)

## combine and plot
series <- cbind(N.random, one.over.f.pattern, N.f)
...
Imagine that the Ohio (USA) environment of our Song Sparrows reveals *environmnetal stochasticity*, that is, random-like fluctuations that we do not understand. Next imagine that this environmnetal stochasticity affects Song Sparrow growth rate. If we want to simulation future sparrow population dynamics, we may want to mimic the type and magnitude of this underlying variation. We might do that by using our original annual growth data, but arranged in a reddened series. 
...
```{r plotf, fig.cap="Sparrow population dynamics might show reddened flutuations. On the left, sparrow counts over time, and on the right, the log-log plot of the power spectrum (power vs. frequency). The frequentist confidence interval on the slope of the fitted line suggests that we shouldn't rule out the possibility of a reddened series."}
plot_f(Count)
```
...
```{r Rred, fig.cap="Original and rearranged"}
R.n = length(obs.R) # number of observations

## A random pink noise template
set.seed(2)
one.over.f.pattern <- one_over_f(gamma=1, N=R.n)

## Rearrange the observed R
R.f <- spec_mimic(obs.R, one.over.f.pattern)

## combine and plot
series <- cbind(obs.R, one.over.f.pattern, R.f)
