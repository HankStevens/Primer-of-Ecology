...
## Independent and random attacks {#sec-nicholson-bailey-model}

In keeping with convention of parasitoid models, let us describe dynamics of hosts and their parasitoid enemies with a discrete time model [@nicholson1935bap; @may:1978fk]. This makes sense, as these organisms, typically insects, frequently complete their life cycles together and within a year. Let us pretend that the hosts, $H$, grow geometrically in the absence of parasitoids, such that $H_{t+1} = RH_t$. If we assume that some individuals are attacked and killed by parasitoids, $H_a$, then this rate becomes 
$$
...
``` 
Next we just make a histogram (Fig. @fig-PoisSim) of the number of unattacked hosts, adding a dotted line to show where the true data lie, and a dashed line for the prediction, under the assumption of random and independent attacks, based on the Poisson distribution.
```{r fig-PoisSim, fig.cap="Histogram of simulated host populations, assuming a attacks on hosts are random and independent of each other. The dashed line in the histogram is the expected number of unattacked hosts with random and independent attacxks, whereas the dotted line to the right is the observed number."}
...
Our simulation results (Fig. @fig-PoisSim) indicate that the observed data include far more unattacked hosts than expected by chance.
...
getk(.1,.395)
```
```{r fig-negbin, fig.cap="The negative binomial distribution, where the dispersion parameter $k$ controls variance or breadth the distribution. For a given mean, smaller $k$ causes a greater variance, and results in a higher proportion of zeroes. These $k$ are representative of values from @pacala1990."}
nb.dat <- cbind(
...
$$
H_{t+1} = RH_t\left(1+\frac{aP_t}{k} \right)^{-k}
$$ {#eq-may14a}

$$
P_{t+1} =  H_t - H_t\left(1+\frac{aP_t}{k} \right)^{-k}
$$ {#eq-may14b}
...
The equilibria of the host and parasitoid populations in May's model @eq-may14a, @eq-may14b, are  derived simply, once we decide upon how to describe the condition for an equilibrium (a.k.a. steady state). In models of differential equations, this meant letting $d N/d t =0$. In the discrete case it means letting  $N_{t+1}-N_t=0$. The equilibrium is the value of $N$ when this is true, so $N^*=N_{t+1}=N_t$. Following this train of thought, we have $H^*=H_{t+1} = H_t$ and $P^*=P_{t+1} = P_t$. 

To solve for equilbria, we begin with the expression for hosts @eq-may14a, and solve for equilibrium parasitoid density. In this case, we can divide @eq-may14a both sides by $H^*$.
\begin{align}
...
If we want $H^*$ as well, we can solve for that too. We see that @eq-may14b can be rearranged to show
\begin{align*}
