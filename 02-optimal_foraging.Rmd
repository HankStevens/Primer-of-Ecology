...
If we solved $\delta R / \delta t_r = 0$, we would discover that $R$ will be maximized when it equals the *slope* of gain function. That is,
$$\frac{g(t_r)}{t_t + t_r} = g^\prime(t_r)$$

This predicts that in order to maximize the long-term average rate, we should stay in a patch until the instantaneous rate, $g^\prime(t_r)$ drops to the long term average rate, $R$.
