# Density-dependent growth {#sec-ddgrowth}
```{r include=FALSE}
...
sparrows2 <- sparrows |>
  mutate(
    Rt = log(c(Count[-1]/Count[-length(Count)],NA) ),
...
s2 <- sparrows2 |> drop_na() %>%
  ggplot(aes(Count, Rt)) + geom_text(aes(label=Yr)) +
geom_smooth(method="lm", se=FALSE) +
labs(y=bquote(italic(r[t])),
     x=expression("Sparrow Count ("*italic(N[t])*")"))
s2
```

Song Sparrow *Melospiza melodia* counts, $N$, from 1966--2003 and the relation between observed counts ($N_t$) and annual growth rate ($r_t = N_{t+1}/N_t - 1$) determined from those counts, fit with ordinary least squares regression. See @sec-expo for data source.
:::
...
Density-dependent population growth is the case where the per capita population growth rate depends statistically on the density of the population. When the slope of that relation is negative as it is in @fig-sparrowDD, we call this *negative density-dependence*. Negative density dependence a characteristic of a population undergoing *intraspecific competition*, where individuals of the same species compete for shared resources and have negative effects on their demographic rates. So, \ldots how would we represent this algebraically?^[root: Arabic. *Al-jabr*]

## Overthinking: is that regression trustworthy? {#sec-dd-test}

::: {.callout-note collapse="true" title="Overthinking it"}

The regression in @fig-sparrowDD is a good first look, but it has two problems that undermine its $p$-value. First, $N_t, N_{t+1}, N_{t+2}, \ldots$ are not independent draws -- they are successive states of one stochastic trajectory, so the "observations" going into the regression are serially correlated. Second, if $N_t$ is measured with any sampling error (nearly always true of field counts), that error appears on both sides of the fitted relationship and can, by itself, manufacture an apparent negative slope even with zero true density dependence -- a version of *regression to the mean*.

Current practice replaces the simple regression with a **state-space model** that separates the true population process from the observation error on top of it, then tests density dependence as a comparison of two nested models: a pure random walk (no density dependence, $b=1$) against a model pulled back toward some equilibrium (density dependence, $b<1$),
$$
x_{t+1} = u + b\, x_t + w_t, \qquad w_t \sim N(0, q)
$$
$$
y_t = x_t + v_t, \qquad v_t \sim N(0, r)
$$
where $x_t = \log N_t$ is the true (unobserved) log population size, $y_t$ is the log of what we counted, $w_t$ is real year-to-year process noise, and $v_t$ is observation error. This is a *Gompertz state-space model* [@dennis2006edd], and it fits easily in R with the **MARSS** package.

```{r}
#| message: false
y <- matrix(log(sparrows$Count), nrow = 1)

mod.dd <- list(B = matrix("b"), U = matrix("u"), Q = matrix("q"),
               Z = matrix(1), A = matrix(0), R = matrix("r"),
               x0 = matrix("pi"), tinitx = 0)
mod.di <- mod.dd
mod.di$B <- matrix(1)                     # null: density-independent random walk

fit.dd <- MARSS(y, model = mod.dd, silent = TRUE)
fit.di <- MARSS(y, model = mod.di, silent = TRUE)

coef(fit.dd)$B                            # estimated b
LRT <- 2 * (fit.dd$logLik - fit.di$logLik)
pchisq(LRT, df = 1, lower.tail = FALSE)   # naive chi-square p-value
```

For the sparrow data, $\hat b \approx 0.68$ (below 1, consistent with density dependence), and the likelihood-ratio test gives $p \approx 0.013$ -- not so different from the naive regression's $p \approx 0.012$. That similarity is itself instructive: near the $b=1$ boundary the usual chi-square approximation to the likelihood-ratio statistic is unreliable, for exactly the reason you suspected -- the data are not independent. Dennis and Taper (1994) fixed this with a **parametric bootstrap**: simulate many new time series from the fitted null model ($b=1$), refit both models to each simulated series, and see how often a random-walk population produces an apparent LRT as large as the one we actually observed.

```{r}
#| label: bootMARSS
#| cache: true
set.seed(42)
nboot <- 100    # ~4 minutes to run once; cached thereafter by `freeze: auto`
LRT.boot <- replicate(nboot, {
  ysim <- matrix(MARSSsimulate(fit.di, nsim = 1, tSteps = ncol(y))$sim.data[, , 1], nrow = 1)
  f.dd <- MARSS(ysim, model = mod.dd, silent = TRUE)
  f.di <- MARSS(ysim, model = mod.di, silent = TRUE)
  max(2 * (f.dd$logLik - f.di$logLik), 0)
})
mean(LRT.boot >= LRT)                     # bootstrap p-value
```

The bootstrap $p$-value, $\approx 0.1$, is noticeably larger than either naive answer -- once the null model is allowed to generate data with the same autocorrelation structure as the real thing, an LRT this large is no longer so surprising. For this 38-year series, the honest conclusion is "suggestive, but not strong evidence" of density dependence, not the fairly confident "yes" that OLS implied. This is the general pattern to expect: non-independence inflates apparent significance, so a naive regression's $p$-value usually *overstates* the evidence for density dependence, and a bootstrap-corrected test is the more defensible one to report.

:::

## Continuous logistic growth {#sec-logistic-growth}
...
**Theory alert!** Recall that Alan @hastings2011ssp identified principles of macroscopic theory of population growth, that (i) populations grow exponentially, unless (ii) something prevents exponential growth. Here we see one of those somethings -- negative density dependence.
...
The term *stability* is often an umbrella term for many related ideas such as resilience, resistance, reactivity, and permanence [@chen2001gsl].^[Resistance refers to the tendency to resist change in the first place; resilience refers to the rate of return to an equilibrium; reactivity refers to the rate of change immediately following a perturbation [@neubert1997], and permanence is the degree to which a system does not simplify by entering one of its boundary equilibria.] In this book, we focus on resilience, the tendency for a system to return an equilibrium, or be *attracted toward* an equilibrium, if it is perturbed from it.
...
### Analytical linear stability analysis {#sec-stability-analysis}
...
$$ {#eq-pdlog}
Further, we are interested in the equilibrium, where $N = 1/\alpha$. If we substitute this into eq. @eq-pdlog and simplify, this reduces to 
$$
...
Near $N^*$, population growth rate is $-rx$.^[Because $\Delta y/\Delta x = - r$, i.e. the change in $y$ equals the change in $x$ times the slope.] That is, the rate of change of $x$ (Figs. @fig-stab2) is 
$$
  \frac{dx}{dt} = -rx
$$ {#eq-9}
...
We can use R's minimal symbolic capabilities to get derivatives. Here we get the partial derivative and then evaluate it for the two equilibria (@fig-FNN).
```{r symbolic}
alpha <- 0.01; r <- 1
...
There are many mathematical forms of density-dependent growth, some of which will we describe below. Let's start by decoupling the density-dependence of birth and death rates in logistic growth [@gotelli2001vv]. 

Recall from @sec-expo (density-independent growth) that $r=b-d$. Now we will let $b$ and $d$ be functions of population size $N$ ($b=F(N)$, $d=G(N)$). We could let $F(N)$ and $G(N)$ take a wide variety of different forms. For now, however, let's say that $N$ has *linear* effects on these rates so that 
\begin{align}
b &=b_0 - aN\\
d &= d_0 + cN
\end{align}
which appear in @fig-bd. 

```{r fig-bd, echo=FALSE, fig.cap="Per capita birth and death rates can be functions of density (dashed lines). The carrying capacity (K) occurs at the point where the per capita birth and death rates are equal. The solid line represents the per capita population growth rate, which is zero when $N=K$.", out.width="75%"}
...
Here we explore a simple extension of the logistic model, the *theta-logistic* model, which adds a parameter to increase flexibility and generality.
$$
...
Approximations of eq. @eq-11 that allow $\theta \leq 0$ are possible [@sibly2005rpm], but the interpretation becomes strained.
...
By varying $\theta$, we can change the linear density dependence of the simple logistic model to curvilinear density dependence (@fig-ThetaDD). 
...
$$
\frac{dN}{dt}= rN\left(1-\alpha N\right)\left(1 - \frac{a+\tau}{N+\tau}\right)
$$ {#eq-logallee}
...
We might as well add that there is an expression for $N_{t}$, analogous to $N_{t}=N_{0}\exp(rt)$ for exponential growth. If we integrate eq. @eq-logalpha with respect to time, we have
\begin{equation}
...
## Discrete Density-dependent Growth {#sec-discrete-logistic-growth}
...
Here $r_d$ has the same meaning it did in @sec-expo, where $r_d = b-d-bd$. When $r_d$ is quite small in a slow growing population, this is very similar to the continuous form. However, as $r_d$ becomes larger in a faster growing population, the dynamics can get quite exciting.
...
the $x$-axis. Now we are going to examine $N$ as a function of $r_d$, so $r_d$ is on the $x$-axis. Specifically, we will plot the stable limits or attractors $vs$. $r_d$ (@fig-bifurcation1). What does it mean? For $r_{d} < 2$, there is only a single $N$. This is what we mean by a stable point equilibrium, or point attractor.
...
If we generate simulations where we vary initial population size by a single individual, we find that this can have an enormous impact on the similarity of the dynamics of two populations, and on our ability to predict future population sizes (@fig-DDGChaosInitN). Note how the populations start with similar trajectories, but soon diverge so that they experience different sequences of minima and maxima (@fig-DDGChaosInitN). This is part of what was so upsetting to ecologists about May's 1974 paper --- perhaps even the simplest deterministic model could create dynamics so complex that we could not distinguish them from random [@may1974bpn]. Over time, however, we came to learn that (i) we could distinguish random dynamics from some chaos-like dynamics, and (ii) the hunt for chaos could be very exciting, if most frequently disappointing [@becks2005edc; @constantino1995eit; @kendall1998mpd].
...
```{r fig-DDGChaosInitN, fig.cap="Effects of differences in initial population size on the short term and long term dynamics, and their correspondence, of three populations.", out.width="75%"}
N.init <- c(97,98,99)
t <- 0:30
...
One last issue that we should note is the extent to which our populations are *bounded*. A population may have complex dynamics, but we may be able to characterize a given population by its upper and lower bounds. In spite of the differences created by the initial conditions, the upper and lower bounds of our chaotic populations were very similar (@fig-DDGChaosInitN). In contrast, a population undergoing a random walk, as in our sparrow simulations in @sec-expo, will be unbounded - the variance will increase with time.

Note also as $r_d$ increases (@fig-bifurcation1) the oscillations increase very systematically. One way to see the deterministic nature of chaos is to trace population size from each time point to the next time point. We put $N_t$ on the $x$-axis and $N_{t+1}$ on the $y$-axis, and connect all successive points.  
...
@watkinson1980dsp described a simple model that incorporates nonlinear density dependence which is intuitive, extensible, and is now used to help test modern coexistence theory [@hillerislambers2012rca, @aoyama2022]. 
...
To find the predicted population growth rate with harvesting, we start with Logistic growth rate (solid line) and then substract harvest rate (dashed line) (left figure). This gives us population growth rate in the presence of harvesting (right figure).
:::

If harvesting occurs at a rate of $F<r/2$, the slope of the harvest line is shallower than in @fig-msy1, and we refer to this as *under-harvesting*. If the rate is $F>r/2$, then the slope is steeper than in @fig-msy1, and we refer to this as *over-harvesting*. @roughgarden:1996fr show that under-harvesting is the most ecologically stable, whereas over-harvesting is frequently thought to be the economically optimal approach if we don't care about the long-term wellbeing of the fishing industry.

Typically the economically optimal approach leads to over-harvesting because the marginal income on a fishery is related to the interest rate on invested income,  $F= (r + \rho)/2$, where $\rho$ is the interest rate (usually $\rho$ is set to 0.05). We can substitute this into eq. @eq-FN and solve for $N$ (ignoring $N=0$).
$$
\begin{aligned}
