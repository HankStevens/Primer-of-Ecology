...
1. failing to include survival in the fertility coefficient;
1. introducing a one-year delay in age at first reproduction; and
1. calculating incorrectly the growth rate out of a stage class. 
...
eda_s
```
Using logistic regression provides a straightforward way to estimate size-dependent survival and its uncertainty. We can use both the estimate and it uncertainty in our projections.
...
The larger individuals tend to regress to smaller sizes in subsequent years.
...
It is also true that bigger plants might produce more seeds, although there is not a lot of evidence for that in these data.
...
Here we make functions that will use the above relationships and the data frame to make the kernel. the variable `z` is size in the first year and `zp` is "z prime", size in the next year.

First we create a function to estimate the probability of surviving, $s$,  given a size $z$. This is a back transformation of the logistic function, where
...
Next we create a function to estimate the probability of having a particular "fecundity", which in these models is far more than just offspring production because it encompasses an entire year. It does a lot:
...
Finally, we begin to create our finite analogue of the continuous kernel: our discretized kernel, $K$.  We determine the upper and lower size bounds on log-plant size, the number of discrete cells or size intervals, the bounds of each discrete size interval, and the midpoints of each interval, and the width of interval.
```{r }
...
## Survival kernel
S=s.z(y,params=params)   # survival kernel
## Growth X Survival
P = G # placeholder;redefine P on the next line
for(i in 1:n) P[,i]=G[,i]*S[i]  # growth*survival kernel
## Fecundity kernel
Fec=h*outer(y, y, f.zpz, params=params) 

## the full projection kernel
K=P+Fec # full kernel
```

Finally, we plot the kernels so that we can visualize them and thereby better appreciate what the result is.
```{r kernels, fig.cap="The component kernels G, S, and F, that make up the full demographic projection kernel, K, for smooth cone flower. Unlike projection matrices, kernels scale both axes to start at size = 0 in the lower left corner. The color scaling of the full kernel differs slightly so that we can see the contributions of G, S, and F."}
