...
```{r cflh2, echo=FALSE, fig.cap="Smooth coneflower, *Echinecea laevigata*, is an herbaceous perennial for which size seems to be a useful predictor of demographic rates.", out.width='100%', fig.show='hold'}
...
These are $b_0$ and $b_1$ from the above equation and allow us to describe the line and uncertainty^[In logistic regression, the uncertainty is a function of the mean probability of survival at each size.] we saw above in Fig. \@ref(fig:flowersurv). Next, we extract them and save them. 
...
1. Draw by hand two different types of life cycle graphs (e.g., \@ref(fig:garlic1) and \@ref(fig:cflh2)) for this loggerhead population. Include the matrix elements associated with each transition. 
