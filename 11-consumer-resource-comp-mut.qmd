...
In @sec-comp, we used Lotka-Volterra models of competition and mutualism to learn about some fundamental properties of (-,-) and and (+,+) interactions. Here we add some model complexity to illustrate a more mechanistic approach to modeling greater biological reality.  

In @sec-cr, on consumer-resource interactions, we learned a variety of approaches to representing different types of resources or prey, and different ways consumers consume them. **Please read @sec-cr before proceeding here.** There are a lot of terms and parameters we use here that are more fully explained in @sec-cr. 
...
```{r fig-dN, fig.cap="Mass-specific growth rates of two species across a range of resource concentrations. Note that where growth rates equal death rates, consumer growth is zero (arrows) and this determines each species' $R^*$s.", out.width="75%" }
...
```
From Fig. @fig-dN, we see that at high resource concentrations, species 1 grows faster than species 2. However, at low resource concentrations, species 2 grows faster than species 1. Resource concentrations change as consumers take up resources, so when does the whole system reach an equilibrium? 

Both species (Fig. @fig-dN) increase in biomass as long as resources are high enough to allow growth, whereas resources will decline as consumers grow more and more and take up increasing amounts of resources. Eventually consumer biomass is high enough to drive down resources so low that mass-specific growth rate is zero. This will happen at different resource concentrations for our two species. We see that species 2 has a positive growth rate when species 1 does not (Fig. @fig-dN). This type of resource competition for a single limiting resource was named the $R^*$ rule ("R-star"):
...
In the above model, per capita growth rates were linear functions of resource concentration. It is more common to see a type II [functional response](#sec-fr) for the consumers [@tilman1982] (Fig. @fig-monod), which we described in @sec-cr.
$$\frac{dN_i}{Ndt}= b_i\frac{R}{k_i + R} - d_i$$
```{r fig-monod, fig.cap="The Monod function for per capita resource-dependent growth rates of two different populations. These are a type II functional responses.", out.width="67%"}
...
## Tilman's resource ratio with essential resources {#sec-tilman-resource-ratio-model}
...
can acquire or manufacture in surplus and trade it for a resource or product that is limiting. This is a bi-drectional mutualism (Fig. @fig-mut3) because the flow of \emph{resources} are bidirectional: both species provide a resource to the other [@holland2010]. 
```{r}
#| include: false
...
grViz(bimut) %>%
  export_svg %>% charToRaw %>% rsvg_png("figs/bimut.png")

grViz(umut) %>%
  export_svg %>% charToRaw %>% rsvg_png("figs/umut.png")

grViz(imut) %>%
  export_svg %>% charToRaw %>% rsvg_png("figs/imut.png")
```

::: {#fig-mut3 layout-ncol="3"}

```{r}
#| echo: false
include_graphics("figs/bimut.png")
```

```{r}
#| echo: false
include_graphics("figs/umut.png")
```

```{r}
#| echo: false
include_graphics("figs/imut.png")
```

Bidirection, unidirectional, and indirect mutualisms. Normal arrowhead is a positive effect, whereas an open dot arrowhead is a negative effect. A dashed line indicates a service, and a solid line indicates a consumable resource.
:::
...
escape the eel [@bshary2006icc]. Sometimes these mutualisms are
...
service (Fig. @fig-mut3). This is a uni-directional mutualism because the flow of *resources* is only unidrectional, from one species to the other [@holland2010].
...
Fruit consumption by a single frugivore is governed by a type II functional response as fruit abundance increases. Successful seed dispersal and establishment is governed by a type II functional response as frugivores increase.
:::
...
one of ecologists helping lead the way (Fig. @fig-beverfig). 

```{r fig-beverfig, echo=FALSE, fig.cap="Plants and soil microbes can interact in negative or positive ways. Plants may grow more poorly with their own soil flora than with that of a competitor's.", out.width="75%"}
library(diagram)
...
Fig. @fig-beverfig reflects the following Lotka-Volterra style model of plant species $N_A$ and $N_B$, and soil microbial floras $S_A$ and $S_B$:
  \begin{equation}
...
We will parameterize the model in a way that reveals the importance of the soil floras to coexistence, according to criteria stated above @eq-bevercrit. We will create a vector of parameters, and then test whether the inequality in @eq-bevercrit is true. We start with an absence of a soil flora effect.
