...
### chunk number 1: 
###################################################
setwd("~/Documents/Projects/Book/draft/Stevens-primer-2010/Chap09")
rm(list=ls())
badfonts = FALSE
...
cd1 <- .3
cd2 <- 1.1
ps2  <- matrix(0, nrow=t+1, ncol=2); ps2[1,] <- c(.1,.5); # ps2[1,] <- ps[2,]
for(i in 1:t) ps2[i+1,] <- {p1 <- ps2[i,1] + cd1 * ps2[i,1] * (1 - ps2[i,1]) - md1 * ps2[i,1]
                          p2 <- ps2[i,2] + 
                            cd2 * ps2[i,2] * (1 - ps2[i,1] - ps2[i,2]) - 
                            md2 * ps2[i,2] - cd1 * ps2[i,1] * ps2[i,2]
                          c(p1, p2)}
