...
### Representing more realistic direct mutualisms
One feature of the above representation is that we cannot represent obligate mutualisms. We can do this if we rewrite the above equations in one of the ways we did in Chapter 5, letting $b=r$ and $a=r\alpha$. 

\begin{align*}
\frac{dN_1}{dt} &= N_1\left( b_1 - a_{11} N_1 + a_{12} N_2\right)\\
\frac{dN_2}{dt} &= N_2 \left(b_2 + a_{21}N_1 - a_{22}N_2\right)
\end{align*}
...
Here we write an ODE model to help anaylze dynamics.
```{r echo=TRUE}
mutualism.LV2 <- function(t, y, params){
  n1 <- y[1]; n2 <- y[2]
  with(as.list(params), {
    dn1.dt = n1*(b1 - a11*n1 + a12*n2)
    dn2.dt = n2*(b2 + a21*n1 - a22*n2)
    return(list( c(dn1.dt, dn2.dt) ) )
  })
}

parameters2 <- c(a11 = .002, a22 = .002, a12 = .001, a21=.001,
                b1= -.01, b2= -.01)
with(as.list(parameters2),
     (b1*a21 - b2*a11)/(a11*a22-a12*a21))
```
```{r results='hide', include=FALSE}
t <- 0:100
out <- ode(c(4,4), t, mutualism.LV2, parameters2)
plot(out)
```
